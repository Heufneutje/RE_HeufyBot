...
public class PasteUtils
{
    public static String post(final String data, final String title, String expire)
    {
        try
        {
            ...
            String postData = URLEncoder.encode("key", "UTF8") + "="
                    + URLEncoder.encode(key, "UTF8") + "&"
                    + URLEncoder.encode("description", "UTF8") + "="
                    + URLEncoder.encode(title, "UTF8") + "&" + URLEncoder.encode("paste", "UTF8")
                    + "=" + URLEncoder.encode(data, "UTF8") + "&"
                    + URLEncoder.encode("expiration", "UTF8") + "="
                    + URLEncoder.encode(expire, "UTF8") + "&" + URLEncoder.encode("format", "UTF8")
